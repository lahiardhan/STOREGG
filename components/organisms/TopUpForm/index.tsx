...
export default function TopUpForm(props: TopUpFormProps) {
	...
	const onPaymentItemClick = (payment: PaymentTypes, bank: BanksTypes) => {
		const data = {
			payment,
			bank,
		}
	...
	const onSubmit = () => {
		if(verifyID === '' || bankAccountName === '' || nominalItem === {} || paymentItem === {}) {
			toast.error('Silahkan Isi Semua Data Terlebih Dulu!', {
				theme: "colored"
			})
		} else {
			const data = {
					...
					<div className="row justify-content-between">
						{payments.map((payment) => (
							payment.banks.map((bank) => (
								<PaymentItem 
									onChange={() => onPaymentItemClick(payment, bank)}
									key={bank._id}
									bankID={bank._id}
									type={payment.type}
